<script>

import { goto } from '$app/navigation';
import { resolve } from '$app/paths';
import { auth } from '$lib/firebase';
import { signInWithEmailAndPassword } from 'firebase/auth';
import { PUBLIC_API_URL } from '$env/static/public';

    // State management for form data and UI micro-interactions
    let email = $state("");
    let password = $state("");
    let showPassword = $state(false);
    
    // States for the submission button animation
    let isLoading = $state(false);
    let isSuccess = $state(false);

    // Scroll progress tracking
    let scrollProgress = $state(0);
    let errorMessage = $state("");

    // Toggle password visibility
    const togglePassword = () => {
        showPassword = !showPassword;
    };

    // Handle form submission
    /** @param {SubmitEvent} event */
    const handleLogin = async (event) => {
        event.preventDefault();
        errorMessage = '';
        isLoading = true;

        try {
            // 1. Authenticate with Firebase
            const credential = await signInWithEmailAndPassword(auth, email, password);

            // 2. Get the ID token to authenticate with our backend
            const idToken = await credential.user.getIdToken();

            // 3. Confirm the user exists in our DB and fetch their record
            const res = await fetch(`${PUBLIC_API_URL}/api/me`, {
                method: 'GET',
                headers: {
                    Authorization: `Bearer ${idToken}`
                }
            });

            if (!res.ok) {
                const text = await res.text();
                throw new Error(text || `Could not fetch user (${res.status})`);
            }

            console.log('Login successful!');

            isSuccess = true;

            setTimeout(() => {
                goto(resolve('/profile'))
            }, 1000);
        } catch (err) {
            if (err instanceof Error) {
                errorMessage = err.message;
            } else {
                errorMessage = 'Something went wrong. Please try again.';
            }
            console.error('Login error:', err);
        } finally {
            isLoading = false;
        }
    };

    const handleScroll = () => {
        const winScroll = document.body.scrollTop || document.documentElement.scrollTop;
        const height = document.documentElement.scrollHeight - document.documentElement.clientHeight;
        
        if (winScroll > 10) {
            scrollProgress = (winScroll / height) * 100;
        } else {
            scrollProgress = 0;
        }
    };
</script>


<svelte:window on:scroll={handleScroll} />

<svelte:head>
    <title>Log In | /Blog</title>
    <link href="https://fonts.googleapis.com/css2?family=EB+Garamond:wght@400;500;600;700&family=Inter:wght@400;500;600&family=Source+Serif+4:ital,wght@0,400;0,600;1,400&display=swap" rel="stylesheet" />
    <link href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:wght,FILL@100..700,0..1&display=swap" rel="stylesheet" />
</svelte:head>

<div 
    class="fixed top-0 left-0 h-`2px` bg-primary z-`100` transition-all duration-300 ease-out" 
    style="width: {scrollProgress}%; opacity: {scrollProgress > 0 ? 1 : 0};"
></div>

<div class="min-h-screen bg-surface text-on-surface selection:bg-primary-fixed selection:text-on-primary-fixed font-sans antialiased">
    <header class="fixed top-0 left-0 w-full z-50 bg-surface px-gutter py-6 flex justify-center">
        <div class="max-w-container-max w-full flex justify-center items-center">
            <a class="transition-transform duration-200 hover:scale-105" href={resolve('/')}>
                    <img alt="/Blog Logo" class="h-10 w-auto" src="data:image/png;base64," />
            </a>
        </div>
    </header>

    <main class="min-h-screen flex flex-col justify-center items-center px-margin-mobile pt-24 pb-12">
        <div class="w-full max-w-100 flex flex-col items-center">
            
            <div class="text-center mb-element-gap">
                <h1 class="font-headline-md text-headline-md text-primary mb-2">Welcome back.</h1>
                <p class="font-body-md text-body-md text-secondary max-w-[320px] mx-auto">
                    Log in to your account to continue sharing your stories.
                </p>
            </div>
            
            <div class="w-full bg-surface-container-lowest p-8 lg:p-10 rounded-lg shadow-[0px_4px_20px_rgba(0,0,0,0.04)]">
                <form class="space-y-6" onsubmit={handleLogin}>
                    <div class="space-y-2">
                        <label class="font-label-md text-label-md text-on-surface-variant block" for="email">
                            Email address
                        </label>
                        <input 
                            bind:value={email}
                            class="w-full bg-transparent border-b border-outline-variant py-2 font-body-md text-body-md transition-all duration-200 focus:outline-none focus:border-primary focus:ring-0" 
                            id="email" 
                            name="email" 
                            placeholder="you@example.com" 
                            required 
                            type="email"
                        />
                    </div>
                    
                    <div class="space-y-2">
                        <div class="flex justify-between items-center">
                            <label class="font-label-md text-label-md text-on-surface-variant block" for="password">
                                Password
                            </label>
                        </div>
                        <div class="relative">
                            <input 
                                bind:value={password}
                                class="w-full bg-transparent border-b border-outline-variant py-2 font-body-md text-body-md transition-all duration-200 focus:outline-none focus:border-primary focus:ring-0" 
                                id="password" 
                                name="password" 
                                placeholder="••••••••" 
                                required 
                                type={showPassword ? "text" : "password"}
                            />
                            <button 
                                class="absolute right-0 top-1/2 -translate-y-1/2 text-outline hover:text-primary transition-colors" 
                                type="button" 
                                onclick={togglePassword}
                            >
                                <span class="material-symbols-outlined text-[20px]" style="font-variation-settings: 'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24;">
                                    {showPassword ? 'visibility_off' : 'visibility'}
                                </span>
                            </button>
                        </div>
                    </div>

                    {#if errorMessage}
                        <p class="mt-4 text-center font-label-md text-label-md text-red-600" role="alert">
                            {errorMessage}
                        </p>
                    {/if}
                    
                    <button 
                        class="w-full text-on-primary py-4 rounded-lg font-ui-button text-ui-button hover:opacity-90 active:scale-[0.98] transition-all duration-200 mt-4 flex justify-center items-center gap-2 group {isSuccess ? 'bg-primary-container' : 'bg-primary'}" 
                        type="submit"
                        disabled={isLoading}
                    >
                        {#if isLoading}
                            <span class="material-symbols-outlined animate-spin" style="font-variation-settings: 'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24;">progress_activity</span>
                        {:else if isSuccess}
                            <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24;">check_circle</span>
                        {:else}
                            Log In
                            <span class="material-symbols-outlined text-[18px] group-hover:translate-x-1 transition-transform" style="font-variation-settings: 'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24;">arrow_forward</span>
                        {/if}
                    </button>
                </form>
            </div>
            
            <div class="mt-8 text-center">
                <p class="font-label-md text-label-md text-secondary">
                    Don't have an account? 
                    <a class="text-primary font-bold hover:underline ml-1" href={resolve('/auth/registration')}>Sign Up</a>
                </p>
            </div>
        </div>
    </main>

    <footer class="w-full py-8 px-gutter flex flex-col md:flex-row justify-center items-center gap-4 opacity-60">
        <p class="font-label-md text-label-md text-secondary">© 2024 /Blog Editorial. All rights reserved.</p>
        <div class="flex gap-4">
            <a class="font-label-md text-label-md text-secondary hover:text-primary transition-colors" href="#privacy">Privacy</a>
            <a class="font-label-md text-label-md text-secondary hover:text-primary transition-colors" href="#terms">Terms</a>
        </div>
    </footer>
</div>