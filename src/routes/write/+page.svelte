<script>
	...
	async function persist(status) {
		...
		try {
			...
			if (!postId) {
				...
				const draftPostId = draftResult.id;

				postId = draftPostId;
				result = status === 'public'
					? await updatePost(draftPostId, { ...payload, status: 'public' })
					: draftResult;
			} else {
	...
	// Auto-save debouncer (always saves as draft)
...
<svelte:head>
    <title>Write - /Blog Editorial</title>
